...
def run_fit(y, X):
    ...
    c_inflection_min = math.inf
    k_slope_min = math.inf
    k_slope_max = math.inf
    k_slope_initial = 0
    # x-change: -10:max(x)
    # slope (k): 1 #[2 results in extremely steep curve given date range, reasonable max]
    ...
    if np.isnan(y).sum() < len(y):
        try:
            popt, pcov = curve_fit(nl_func, X, y, maxfev=maxfev)
